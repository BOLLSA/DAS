...
//   purifyAllFriendly     - 净化师被动清除负面效果
    ...
    function consumeNerdJamPending(unit, actionName) {
        ...
        addLog(`👓 行动干扰生效！${unit.cardName} 的${actionName}被无效化！`);
        showToast(`👓 行动干扰！${unit.cardName} 的${actionName}被无效化`);
    ...
    function purifyAllFriendly(side) {
        ...
        for(let c of gameState.players[side].hand) {
            if(c.disabled) { c.disabled = false; c.disabledBy = null; c.disabledTurns = 0; addLog(`手牌 ${c.name} 的禁用被净化解除。`); }
        }
    }
